...
window.loginWithSocial = async function (providerName) {
  const { error } = await supabaseClient.auth.signInWithOAuth({
    ...
    options: {
      redirectTo: `https://sarmad-tabassum.github.io/post-app-with-supabase-smit/dashboard.html`,
    },
  });
...
if (window.location.pathname.includes("forgot.html")) {
  ...
  sendResetBtn.addEventListener("click", async (e) => {
    ...
    const { error } = await supabaseClient.auth.resetPasswordForEmail(
      emailValue,
      {
        redirectTo: `https://sarmad-tabassum.github.io/post-app-with-supabase-smit/update-password.html`,
      },
    );
